...
- Decor: drag to draw decor box stored as `{ type: "decor", rectangle, types, n_row, n_col, fps, n_frames, event_name, filename }`.
...
- Select a decor object to edit `types`, `n_row`, `n_col`, `fps`, `n_frames`, `event_name`, and `filename`.
...
- `event_name` is only used when `types = "interact"`; for other types it is automatically kept empty.
